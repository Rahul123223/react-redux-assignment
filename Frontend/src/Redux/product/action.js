...
export const getProductData = (sort, category, page) => (dispatch) => {
  ...
  if (category.length == 0) {
    ...
    fetch(
      `https://powerful-thicket-30637.herokuapp.com/products?&order=${sort}&page=${page}`
    )
      .then((res) => res.json())
    ...
    dispatch(getProductLoading());
    fetch(
      `https://powerful-thicket-30637.herokuapp.com/products?category=${category}&order=${sort}&page=${page}`
    )
      .then((res) => res.json())
      .then((res) => dispatch(getProduct(res)))
      .catch((err) => dispatch(getProductError(err)));
